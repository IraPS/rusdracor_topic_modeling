...
infolder = '/Users/IrinaPavlova/Desktop/Uni/Бакалавриат/2015-2016/Programming/github desktop/RusDraCor/Ira_Scripts/test_tei' ## папка с TEI/XML #'testtei'
results = './speech_corpus'
...
def parse_xml(path, filename):
    ...
    if len(list(root.iterfind('.//tei:div[@type="act"]', ns))) > 0:
        print("ACTS", name, '\n')
        n = 0
        for div in root.iterfind('.//tei:div[@type="act"]', ns):
            print(div)
            n += 1
            textbyplaybydiv = codecs.open(results+'/byplay/byact/'+name+str(n)+'.txt','a','utf-8')
            ## в этом цикле мы обходим все <sp> в XML-документе в
            #for sp in root.iterfind ('.//tei:sp', ns):
            print(len(list(div.iterfind('.//tei:sp', ns))))
            for sp in div.iterfind('.//tei:sp', ns):
                print(sp)
                ...
                if notmult:
                    ...
                    textbyplaybydiv.write(speechtext)
        ...
        for sp in root.iterfind('.//tei:sp', ns):
            ...
            if notmult:
                ...
                textbyplay = codecs.open(results+'/byplay/byplay/'+name+'.txt','a','utf-8')
                textbyauthor = codecs.open(results+'/byauthor/'+author+'.txt','a','utf-8')
                textbyplaybydiv = codecs.open(results+'/byplay/byact/'+name+'.txt','a','utf-8')
                textbycharacter = codecs.open(bycharacter+speechcode+'.txt','a','utf-8')
                textbygender = codecs.open(bysex+speakersex+'.txt','a','utf-8')
                textbyplay.write(speechtext)
                textbyauthor.write(speechtext)
